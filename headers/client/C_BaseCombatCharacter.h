...
public:
	...
	bool IsAbleToSee(const CBaseEntity* visible, FieldOfViewCheckType checkFOV);	// Visible starts with line of sight, and adds all the extra game checks like fog, smoke, camo...
	...
	virtual bool IsInFieldOfView(CBaseEntity* visible) const;	// Calls IsLookingAt with the current field of view.  
	...
	virtual bool IsLineOfSightClear(CBaseEntity* visible, LineOfSightCheckType checkType = IGNORE_NOTHING) const;// strictly LOS check with no other considerations
